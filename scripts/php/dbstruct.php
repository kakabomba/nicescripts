...
<title>compare mysql database structure</title>
</head>
<body>
<?php
error_reporting(0);
...
<?php
		if ($_REQUEST['change']) {
			$tmp = $_REQUEST['struct1'];
			$_REQUEST['struct1'] = $_REQUEST['struct2'];
			$_REQUEST['struct2'] = $tmp;
			}
		?>
<form method="post">
	<table width="1"><tr valign="bottom">
		<td><textarea name="struct1" cols="50" rows="10"><?php echo $_REQUEST['struct1']?></textarea></td>
		<td><input type="submit" name="change" value="<->"/></td>
		<td><textarea name="struct2" cols="50" rows="10"><?php echo $_REQUEST['struct2']?></textarea></td>
	</tr>
	<tr>
		<td colspan="3" align="right"><input value="compare" type="submit"/></td>
	</tr>
	<tr>
		<?php
		if (preg_match("/[\r\n]+-- Database: `([\w]+)`[\r\n]+/im",$_REQUEST['struct1'],$name1)) {
			$name1 = "`$name1[1]`";
			}
		else
			$name1 = "LEFT";
		if (preg_match("/[\r\n]+-- Database: `([\w]+)`[\r\n]+/im",$_REQUEST['struct2'],$name2)) {
			$name2 = "`$name2[1]`";
			}
		else
			$name2 = "RIGHT";
		?>
		<td colspan="3"><strong><?php echo $name1 . '->'  . $name2 ?></strong></td>
		</tr>
	<tr>
		<td colspan="3"><em><?php echo
		"Below is SQL requests that have to be applied to $name1 database in order to
obtain $name2 database" ?></em></td>
	</tr>
	<tr>
		<td colspan="3"><textarea rows="7" style="width: 100%;"><?php
		$updater = new dbStructUpdater();
		$res = $updater->getUpdates($_REQUEST['struct1'], $_REQUEST['struct2']);
		echo implode(";\n",$res);
		?></textarea></td>
	</tr>
</table>
